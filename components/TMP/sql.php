	...
	if(true === $result){
		...
		if($result->fetch_assoc()["compte_supprime"]){
			...
			if(false === $query->execute()){
				$conn->close();
				return console_log("Echec de mis à jour de la base de donnée.");
			}
			$conn->close();
		}
		else {
		//s'il n'a pas été supprimé, on renvoie un message d'erreur
			$conn->close();
			return console_log("Le compte existe déjà.");
		}
	}
	else{
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de création d'un nouvel utilisateur.");
		}
		$conn->close();
	}
	return 0;
...
function delete_user(string $email)
{
	...
	if(true === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression du compte.");
		}
		$conn->close();
	}
	else {
		...
		$conn->close();
		return console_log("Le compte que vous voulez supprimé n'existe pas.");
	}
	return 0;
}
...
function get_user(string $email)
{
	...
	if($result){
		...
		return console_log("Le compte n'existe pas.");
	}
}
...
function update_user($email,$options)
{
	...
	if($config['prenom'] != NULL){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à jour du prénom de l'utilisateur.");
		}
	}
	...
	if($config['nom'] != NULL){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à jour du nom de l'utilisateur.");
		}
	}
	...
	if($config['mot_de_passe'] != NULL){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à jour du mot de passe de l'utilisateur.");
		}
	}
	...
	if($config['description'] != NULL){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à jour de la description de l'utilisateur.");
		}
	}
	...
	if($config['role'] != NULL){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à jour du rôle de l'utilisateur.");
		}
	}
	$conn->close();
	return 0;
...
function add_writing_right(string $email, string $nom_tag)
{
	...
	if(true === $result){
		...
		if($result->fetch_assoc()["ecriture"]){
			...
			return console_log("L'utilisateur a déjà le droit d'écriture sur ce tag.");
		}
		else{
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec d'attribution du droit d'écriture sur le tag");
		}
		$conn->close();
	}
	return 0;
...
function add_reading_right(string $email, string $nom_tag)
{
	...
	$query->execute();
	$result = $query->get_result();
	if($result === true && $result->fetch_assoc()["lecture"]){
		//s'il en a, on regarde s'il a le droit de lecture
		//si c'est le cas, on renvoie un message d'erreur
		$conn->close();
		return console_log("L'utilisateur a déjà le droit de lecture sur ce tag.");
	}
	else{
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec d'attribution du droit de lecture sur le tag");
		}
		$conn->close();
	}
	return 0;
...
function modify_rights(string $email, string $nom_tag, int $ecriture, int $lecture)
{
	if($ecriture == 1 && $lecture != 1){
		return console_log("L'attribution de droit du droit d'écriture doit impliqué le droit de lecture sur le tag.");
	}
	...
	$query->execute();
	$result = $query->get_result();
	if(true === $result && ($result->fetch_assoc()["ecriture"] != $ecriture || $result->fetch_assoc()["lecture"] != $lecture)){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de modification des droits sur le tag");
		}
		$conn->close();
	}
	else{
		...
		$conn->close();
		return console_log("L'utilisateur n'a aucun droits sur ce tag ou aucune modification n'est nécessaire.");
	}
	return 0;
}
...
function delete_rights(string $email, string $nom_tag)
{
	...
	if(true === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression des droits sur le tag");
		}
		$conn->close();
	}
	else{
		//sinon on renvoie un message d'erreur
		$conn->close();
		return console_log("L'utilisateur n'a aucun droits sur ce tag");
	}
	return 0;
}
...
function add_file(string $source, string $nom_fichier, string $email, float $taille, string $type, string $extension)
{
	...
	if(false === $result){
		...
		//sinon on renvoie un message d'erreur
		$conn->close();
		return console_log("Le fichier existe déjà.");
	}
	return 0;
}
...
function delete_file(string $nom_fichier)
{
	...
	if(true === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression du fichierde la table fichier.");
		}
		$query = $conn->prepare("DELETE FROM fichier_supprime WHERE nom_fichier = ?");
		$query->bind_param("s",$nom_fichier);
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression du fichier de la table fichier supprimé.");
		}
		if(false === unlink($path)){
			$conn->close();
			return console_log("Le fichier n'a pas pu être supprimé du serveur.");
		}
		$conn->close();
	}	
	else{
		//sinon on renvoie un message d'erreur
		$conn->close();
		return console_log("Le fichier n'a pas pu être supprimé car il n'existe pas.");
	}
	return 0;
}
...
function get_file(string $nom_fichier)
{
	...
	if(true === $result){
		...
		return console_log("Le fichier n'existe pas.");
	}
}
...
function add_link(string $nom_fichier, string $nom_tag)
{
	...
	if(false === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec d'attribution du tag au fichier.");
		}
		$conn->close();
	}
	else{
		//sinon on renvoie un message d'erreur
		$conn->close();
		return console_log("Ce tag est déjà attribué au fichier.");
	}
	return 0;
}
...
function delete_link(string $nom_fichier, string $nom_tag)
{
	...
	if(false === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression du tag du fichier.");
		}
		$conn->close();
	}
	else{
		//sinon on renvoie un message d'erreur
		$conn->close();
		return console_log("Ce tag n'est pas associé à ce fichier.");
	}
	return 0;
}
...
function get_link($nom_fichier)
{
	...
	if(true === $result){
		...
		return console_log("Echec de récupération des tags associés au fichier.");
	}
}
...
function add_tag(string $nom_tag, string $nom_categorie_tag = "autres")
{
	...
	if(false === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec d'association du tag à la catégorie.");
		}
		$conn->close();
	}
	else{
		...
		$conn->close();
		return console_log("Ce tag est déjà associé à cette catégorie.");
	}
	return 0;
}
...
function modify_tag_name(string $nom_tag, string $nouveau_nom_tag)
{
	...
	if (false === $query->execute()){
		$conn->close();
		return console_log("Echec de mise à jour du nom du tag dans la table tag.");
	}
	...
	if (false === $query->execute()){
		$conn->close();
		return console_log("Echec de mise à jour du nom du tag dans la table appartenir.");
	}
	...
	if (false === $query->execute()){
		$conn->close();
		return console_log("Echec de mise à jour du nom du tag dans la table attribuer.");
	}
...
function delete_tag(string $nom_tag)
{
	...
	if(true === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression du tag dans la table tag.");
		}
		...
		$conn->close();
		return console_log("Ce tag n'existe pas.");
	}
	return 0;
}
...
function add_tag_category(string $nom_categorie_tag)
{
	...
	if(false === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de création d'une catégorie de tag.");
		}
		$conn->close();
	}
	else{
		//sinon on renvoie un message d'erreur
		$conn->close();
		return console_log("La catégorie de tag existe déjà.");
	}
	return 0;
}
...
function delete_tag_category(string $nom_categorie_tag)
{
	if($nom_categorie_tag == "autres"){
		return console_log("La catégorie 'autres' ne peut pas être supprimé.");
	}
	...
	if(false === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de suppression d'une catégorie de tag.");
		}
		$query = $conn->prepare("UPDATE tag SET nom_categorie_tag = 'autres' WHERE nom_categorie_tag = ?");
		$query->bind_param("s",$nom_categorie_tag);
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à jour de la table tag.");
		}
		$conn->close();
	}
	else{
...
function get_tag_category()
{
	...
	if(true === $result){
		...
		return console_log("Echec de récupération des catégories de tags.");
	}
}
...
function modify_tag_category_name(string $nom_categorie_tag, string $nouveau_nom_categorie_tag)
{
	...
	if (false === $query->execute()){
		$conn->close();
		return console_log("Echec de mise à jour du nom de la catégorie de tag dans la table categorie_tag.");
	}
	...
	if (false === $query->execute()){
		$conn->close();
		return console_log("Echec de mise à jour du nom de la catégorie de tag dans la table tag.");
	}
	$conn->close();
	return 0;
}
...
function basket_file(string $nom_fichier)
{
	...
	if(false === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de mise à la corbeille du fichier.");
		}
		$conn->close();
	}
	else{
		//sinon, on renvoie un message d'erreur
		$conn->close();
		return console_log("Le fichier est déjà dans la corbeille.");
	}
	return 0;
}
...
function get_basket_file(string $email = NULL)
{
	...
	if($email == NULL){
		...
		if(true === $result){
			...
			$conn->close();
			return console_log("Erreur de récupération des fichiers supprimés");
		}
	}
	else{
...
function recover_file(string $nom_fichier)
{
	...
	if(true === $result){
		...
		if(false === $query->execute()){
			$conn->close();
			return console_log("Echec de restauration du fichier.");
		}
		$conn->close();
	}
	else{
		//sinon, on renvoie un message d'erreur
		$conn->close();
		return console_log("Le fichier n'est pas dans la corbeille.");
	}
	return 0;
}
...
function basket_check()
{
	...
	if(true === $result){
		...
		$conn->close();
		return console_log("Echec de récupération des fichiers.");
	}
	return 0;
}
